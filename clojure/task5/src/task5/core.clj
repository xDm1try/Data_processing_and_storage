...
(defn dijkstra [graph source destination]
  (loop [distances (assoc (zipmap (keys graph) (repeat inf)) source 0)
         current source
         adjacent (keys (get graph current)) 
         unvisited (apply hash-set (keys graph))
         paths (zipmap (keys graph) (repeat nil))]
    (if (empty? unvisited) 
      ...
      (if (empty? adjacent)
        ...
        (let [cdst (get distances current) 
            ...
            (map
             (fn [param1] (if
                           (> (deref (param1 :tickets)) 0)
                            (get param1 :price)
                            9999)) (vals %))) (vals simplified-graph))))) 
