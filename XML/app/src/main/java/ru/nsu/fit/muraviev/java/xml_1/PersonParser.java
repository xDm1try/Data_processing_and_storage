...
public class PersonParser {
  ...
  public Map<String, PersonData> parse(InputStream stream) throws XMLStreamException {
          ...
          switch (reader.getLocalName()) {
            ...
            case "surname":
              for (int i = 0; i < reader.getAttributeCount(); i++) {
                if (reader.getAttributeLocalName(i).equals("value")) {
                  assert info != null;
                  info.secondName = reader.getAttributeValue(i).trim();
                } else {
            ...
            case "family", "family-name":
              assert info != null;
              info.secondName = reader.getElementText().trim();
              break;
            case "gender":
              if (reader.getAttributeCount() > 0) {
                for (int i = 0; i < reader.getAttributeCount(); i++) {
  ...
  private HashMap<String, PersonData> normalize(ArrayList<PersonData> data, Integer peopleCount) {
    HashMap<String, PersonData> id_records = new HashMap<>();
    ArrayList<PersonData> temp_records = new ArrayList<>();

    System.out.println("=== Normalizing ===");

    // fill up records with known id
    for (PersonData i : data) {
      if (i.id != null) {
    ...
    assert id_records.values().parallelStream().allMatch(x -> x.firstName != null && x.secondName != null);
    assert temp_records.parallelStream().allMatch(x -> x.firstName != null && x.secondName != null);

    data = temp_records;
    temp_records = new ArrayList<>();

    // merge all people that hasn't namesakes
    for (PersonData p : data) {
      List<PersonData> found = findInRecords((PersonData x) -> x.firstName.equals(p.firstName) && x.secondName.equals(p.secondName),
          id_records.values());
      if (found.size() == 1) {
        PersonData foundPerson = found.get(0);
        foundPerson.merge(p);
    ...
    for (PersonData person : data) {
      if (person.siblingsId != null) {
        ...
        List<PersonData> found = findInRecords(
            x -> {
  ...
  private void spouseMerging(HashMap<String, PersonData> id_records) {
    ...
    for (var p : id_records.values()) {
      ...
      PersonData s = id_records.get(spouseId);
      s.spouseId = p.id;
    ...
    for (var p : id_records.values()) {
      ...
      List<PersonData> f = findInRecords(x -> finalSpouseName.equals(x.getFullName()), id_records.values());
      if (f.size() < 1) {
        continue;
      }

      PersonData s = id_records.get(f.get(0).id);
      s.spouseId = p.id;
      if (p.gender.equals("M") && p.wifeId == null) {
        p.wifeId = s.id;
        s.husbandId = p.id;
  ...
  private void childrenAssertion(HashMap<String, PersonData> id_records) {
    System.out.println("=== Children assertion ===");
    for (String key : id_records.keySet()) {
      PersonData p = id_records.get(key);
      p.childrenId.addAll(p.sonsId);
      p.childrenId.addAll(p.daughtersId);
      for (String s : p.daughtersName) {
        List<PersonData> f = findInRecords((PersonData x) -> s.equals(x.firstName + " " + x.secondName),
            id_records.values());
        if (f.size() >= 1) {
          PersonData daughter = f.get(0);
          if (daughter != null) {
      ...
      for (String childId : p.childrenId) {
        PersonData child = id_records.get(childId);
      ...
      if (p.childrenCount != null) {
        if (!p.childrenCount.equals(p.childrenId.size())) {
          System.out.println(p.firstName + " " + p.secondName + " " + p.childrenId.size() + " " + p.childrenCount);
        }
        try {
  ...
  private void siblingsAssertion(HashMap<String, PersonData> id_records) {
    ...
    for (String key : id_records.keySet()) {
      ...
      for (String s : p.sistersName) {
        List<PersonData> f = findInRecords(x -> s.equals(x.firstName + " " + x.secondName),
            id_records.values());
        if (f.size() >= 1) {
          PersonData sister = f.get(0);
          if (sister != null) {
  ...
  private void genderAssertion(HashMap<String, PersonData> id_records) {
    ...
    for (var p : id_records.values()) {
      if (p.gender == null) {
        if (p.wifeId != null || p.wifeName != null) {
          ...
          PersonData pp = id_records.get(p.spouseId);
          if (pp.gender != null) {
  ...
  private List<PersonData> findInRecords(Predicate<PersonData> pred, Collection<PersonData> coll) {
    return coll.
        parallelStream().
        filter(pred).
